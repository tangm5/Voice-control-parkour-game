//ready_flag for detecting guest user ready
//i for ready
var ready_flag = 0;
// start_flag for detecting host start
// 1 for start
var start_flag = 0;


// verify the audio input is start
var start_verify = 0;
// detect how many user in the room
// 1 for 1
// 0 for two users
var alive_flag;
// establish a new channel websocket bridge
...
var BLOCK_WIDTH_MIN = canvas_width / 40;
var BLOCK_WIDTH_MAX = canvas_width / 10;
...
function twoUser(user_id, alive, room_id) {
    USER_ID = user_id;
    ...
    // webSocketBridge.stream('intval').send({coord_x: myGameCharacter.x, coord_y: myGameCharacter.y})
...
// ##################################################
// TODO: ready helper func
// ##################################################
function ready(){
    ready_flag = 1;
// ##################################################
// TODO: start helper func
// ##################################################
}
function start(){
    start_flag = 1;
  ...
  // user_id: id for logged-in user in the database
    ...
    if(map_status==0){
        ...
        while(cur_len<MAP_LEN){
            ...
            if(Math.random()<bomb_exist_probability && bomb_exist_flag<=2){
                myBomb.push(
                new component_static(block_width, 
                                    block_width, 
                                    BOMB_PATH, 
...
function beforeBackhome(user_id, winner, loser) {
    ...
    $.ajax({
        method: "post", 
        url: 'http://127.0.0.1:8000/game/game/died/' + user_id + '/',
        data: {
